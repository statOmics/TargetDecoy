...
#'     interactively.
#' @param maxLength `numeric` indicating the maximum number of dots shown in the PP plot. If `maxLength` is larger than the number of target scores, a dot in the PP plot corresponds with each target score in the object. 
...
evalTargetDecoys <- function(object,
                             ...
                             log10 = TRUE, nBins = 50, maxLength = 1000) {
...
evalTargetDecoysPPPlot <- function(object,
    ...
    # create PP-plot
    p <- .ppPlot(data, maxLength = maxLength)
...
.ppPlot <- function(data, ylim = NULL, maxLength=1000) {
    ppData <- .ppData(data, maxLength=maxLength)
    df <- ppData$df
    pi0 <- ppData$pi0
